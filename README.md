...
1. 배경 제거
1.1. 배경을 지울 이미지들이 있는 폴더가 input_path, 배경을 지운 결과 이미지들을 저장할 폴더가 output_path입니다.
1.2. 사용 시 해당 파일을 여시고, 아래에서 input_path 및 output_path를 처리할 이미지가 있는 폴더 및 저장할 폴더로 변경해주세요.
1.3. 그 후, 터미널에서 `python background_removal.py`를 입력하시면 자동으로 처리됩니다.
